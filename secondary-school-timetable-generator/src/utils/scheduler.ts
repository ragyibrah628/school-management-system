  ...
  const placeSingleLesson = (lesson: LessonItem): boolean => {
    ...
    for (const d of days) {
      for (const pId of periodIds) {
        ...
        // HEURISTIC A: Spread subjects. Penalize if class has this subject already on this day
        const dayCount = classSubjectDayCount[lesson.classId][lesson.subjectId][d];
        if (dayCount > 0) {
          score -= dayCount * 40; // Heavy penalty
        }

        // HEURISTIC B: Encourage compact teacher schedules (optional/future)
        // For simplicity, prefer earlier periods in the day so the timetable builds top-down
  ...
  const placeDoubleLesson = (lesson: LessonItem): boolean => {
    ...
    for (const d of days) {
      for (const pair of consecutivePeriods) {
        ...
        let score = 100;
        const dayCount = classSubjectDayCount[lesson.classId][lesson.subjectId][d];
        if (dayCount > 0) score -= dayCount * 40; // Prefer not having this subject at all on this day, so the double period is the ONLY time they see it
