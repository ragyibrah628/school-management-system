    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 rounded-2xl shadow-sm border gap-4 print:hidden">
        ...
        <button onClick={()=>handlePrint(viewType==='class'?'class':'teacher')} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-black">
          <Printer size={16} /> {viewType==='class' ? 'Print Class (A4 Landscape)' : viewType==='my_teaching' ? 'Print My Timetable (A4 Portrait)' : 'Print Teacher (A4 Portrait)'}
        </button>
      </div>
                      ...
                      >
                        {!cell ? (
                          ...
                          <div className="leading-tight">
                            <div className="font-bold text-[13px] leading-tight ${isPS ? 'text-slate-700' : isActivity ? 'text-indigo-700' : 'text-slate-900'}" style="font-family: Arial, sans-serif;">{subjectName}</div>
                            {lowerText && <div className="text-[11px] font-semibold text-slate-700 truncate" style="font-family: Arial, sans-serif;">{lowerText}</div>}
                          </div>
                        )}
                        {viewType==='class' && cell && (
                          <button
